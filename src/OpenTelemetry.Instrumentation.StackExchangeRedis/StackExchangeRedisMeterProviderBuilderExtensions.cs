...
public static class StackExchangeRedisMeterProviderBuilderExtensions
{
    ...
    /// </summary>
    /// <remarks>Note: A <see cref="IConnectionMultiplexer"/> will be resolved using the
    /// application <see cref="IServiceProvider"/>.</remarks>
    ...
    /// Enables automatic data collection of outgoing requests to Redis.
    /// </summary>
    /// <remarks>Note: If an <see cref="IConnectionMultiplexer"/> is not supplied
    /// using the <paramref name="connection"/> parameter it will be
    /// resolved using the application <see cref="IServiceProvider"/>.</remarks>
    /// <param name="builder"><see cref="MeterProviderBuilder"/> being configured.</param>
    ...
    private static MeterProviderBuilder AddRedisInstrumentationSharedServices(
        this MeterProviderBuilder builder)
    {
        Guard.ThrowIfNull(builder);
